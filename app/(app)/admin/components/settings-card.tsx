...
export function SettingsCard({ initialSettings }: SettingsProps) {
  ...
  const [themes, setThemes] = useState<{ id: string; name: string }[]>([
    { id: initialSettings.defaultTheme, name: initialSettings.defaultTheme },
  ])
  ...
  useEffect(() => {
    fetch('/api/themes')
      .then((res) => res.json())
      .then((data) => {
        const nextThemes = Array.isArray(data.themes) ? data.themes : []
        if (nextThemes.length === 0) return

        const hasDefaultTheme = nextThemes.some(
          (theme: { id: string; name: string }) => theme.id === initialSettings.defaultTheme
        )

        setThemes(
          hasDefaultTheme
            ? nextThemes
            : [{ id: initialSettings.defaultTheme, name: initialSettings.defaultTheme }, ...nextThemes]
        )
      })
      .catch(() => {})
  ...
  const handleToggleGithub = async () => {
    ...
    try {
      ...
      console.error(error)
      alert(
        'Failed to toggle GitHub. Make sure AUTH_GITHUB_ID and AUTH_GITHUB_SECRET are set in .env.local'
      )
    } finally {
      setIsPending(null)
    }
  ...
  const handleToggleGoogle = async () => {
    ...
    try {
      ...
      console.error(error)
      alert(
        'Failed to toggle Google. Make sure AUTH_GOOGLE_ID and AUTH_GOOGLE_SECRET are set in .env.local'
      )
    } finally {
      setIsPending(null)
    }
        ...
        <div className="space-y-5 p-6">
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              Default theme for code highlighting across all pastes.
            </p>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="number"
              value={maxPasteSize}
              onChange={(event) => setMaxPasteSize(parseInt(event.target.value, 10) || 1048576)}
              className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
              min={1024}
              max={52428800}
            />
            <p className="text-xs text-muted-foreground">
              Maximum size per paste in bytes. Default: 1,048,576 (1 MB).
            </p>
          </div>
